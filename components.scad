...
module arduinoMock(){
		...
		for (x = [0:12], y=[-1,1]) {
			*%translate([-arduinoSize[0]/2+(x+3/4)*INCH/10, 0, arduinoSize[2]/2+y*(arduinoSize[2]/2-INCH/20)])rotate([90,0,0])cylinder(r=INCH/30, h=arduinoSize[1]+.2,center = true,$fn=20);
		}
}
...
module gyroMock(){
	difference(){
		union() {
			...
			for (x = [0:7]) {
				%translate([-gyroSize[0]/2+(x+1/2)*INCH/10, gyroSize[1]/2-INCH/20, gyroSize[2]/2])cylinder(r=INCH/30, h=gyroSize[2]+.2,center = true,$fn=20);
			}
		}
		for (i = [-1,1]) {
			translate([-i*(gyroSize[0]/2-gyroHoleRad-gyroHolePadding),-gyroSize[1]/2+gyroHoleRad+gyroHolePadding,0])cylinder(r=gyroHoleRad, h=gyroFullThickness+.1, $fn=40);
...
module gyroHull(){
	*translate([0,0,strongThicknessHV[1]/2])cube([25,20,strongThicknessHV[1]], center = true);
		translate([0,0,gyroFullThickness/2-0.01])cube(gyroSize+minimumThicknessHV[0]*[1,1,0]+[0,0,gyroFullThickness-gyroSize[2]-0.1,1], center = true);
...
rxSize=[15.2,23,1.6];
rxAntennaSize=[1,30,1.8];
rxFullThickness = 2.4;

module rxMock(){
	difference() {
		//union() {
			translate([0,0,rxFullThickness/2-0.01])cube(rxSize+[0,0,rxFullThickness-rxSize[2]],center = true);
	
		//}
		translate([0, rxSize[1]/2-13.5, rxFullThickness/2-rxSize[2]/2])cube([rxSize[0],1,rxFullThickness-rxSize[2]] , center = true);
	...
	translate([rxSize[0]/2-rxAntennaSize[0],rxSize[1]/2+minimumThicknessHV[1],rxFullThickness-rxSize[2]])rotate([2,0,90])cube(rxAntennaSize+[0,0,0]);
	translate([rxSize[0]/2-rxAntennaSize[0],rxSize[1]/2-minimumThicknessHV[1],rxFullThickness-rxSize[2]])cube([rxAntennaSize[0],2*rxAntennaSize[0],rxAntennaSize[2]]);

}


module rxHull(){
 	translate([0,0,rxFullThickness/2-0.01])cube(rxSize+[2*minimumThicknessHV[1],2*minimumThicknessHV[1],rxFullThickness-rxSize[2]-0.01],center = true);
}
...
module motorMock(){
	union() {
	translate([0,0,0])cylinder(r=motorRHBottom[0], h=motorRHBottom[1],$fn=40);
	intersection() {
		translate([0,0,motorRHBottom[1]-motorRHMiddle[1]])cylinder(r1=motorRHBottom[0],r2=motorRHMiddle[0], h=motorRHMiddle[1],$fn=40);
		translate([-motorRHBottom[0]+minimumThicknessHV[0],0,0])cube([2*motorRHBottom[0]-2*minimumThicknessHV[0],motorRHTop[0],motorRH[1]]);
	}
	difference() {
		translate([0,0,motorRHBottom[1]-strongThicknessHV[0]])cylinder(r=motorRHTop[0], h=motorRHTop[1]+strongThicknessHV[0]+.01,$fn=40);
		*translate([0,0,motorRHBottom[1]-strongThicknessHV[0]-.01])cylinder(r=motorRHTop[0]-strongThicknessHV[1], h=strongThicknessHV[0],$fn=40);	
	}

	translate([0,0,motorRHBottom[1]+motorRHTop[1]])cylinder(r=motorRHPin[0], h=motorRHPin[1],$fn=20);
	}
}
...
module motorHull(){
	...
	union() {
		difference() {
			union() {
				cylinder(r=motorRHBottom[0]+strongThicknessHV[0], h=motorRHBottom[1],$fn=40);
				translate([0,0,motorRHBottom[1]-motorRHMiddle[1]]){
					intersection() {
						translate([0,0,-strongThicknessHV[0]])cylinder(r=motorRH[0]+strongThicknessHV[0], h=motorRH[0]+strongThicknessHV[0],$fn=40);
						translate([0,0,motorRHMiddle[1]])sphere(r=motorRH[0]+strongThicknessHV[0],$fn=40);
				}
			}
			translate([0,0,motorRHBottom[1]])cylinder(r=motorRH[0]+strongThicknessHV[0], h=motorRHTop[1],$fn=40);
		}
			rotate([0, 0, -90])
			for (a = [45:45:315]) {
					hull() {
						translate([0,0,motorRH[1]])rotate([140, 0, a])cylinder(r1=0,r2=ventHoleRad, h=2*motorRH[0],$fn=40);	
						translate([0,0,motorRH[1]-strongThicknessHV[1]-ventHoleRad])rotate([90, 0, a])cylinder(r1=0,r2=ventHoleRad, h=2*motorRH[0],$fn=40);	
					}
			}
		}
